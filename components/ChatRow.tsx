...
function ChatRow({id}: Props) {
    ...
    //To remove the chat in firestore so it will delete it from the chatrow as well
    const removeChat = async()=>{
          await deleteDoc(doc(db, 'users', session?.user?.email! ,'chats', id))
          router.replace("/") //Deleting the chat and going to home
    }
  return (
    ...
    <p className="flex-1 hidden md:inline-flex truncate">
       {/* Showing the text initial from database or new chat instead */}
